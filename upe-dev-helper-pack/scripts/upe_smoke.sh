...
echo "2) Compile (mentor, primary=md, secondaries=pdf,docx)"
RESP=$($CURL -X POST "$API/upe/compile" -H "Content-Type: application/json" -d '{
  ...
  "mode":"mentor",
  "seed":42,
  "artifact":{"primary":"md","secondaries":["pdf","docx"]},
...
echo "4) Download artifacts (md, pdf, docx)"
for k in md pdf docx; do
  OUT="art_${MID}.${k}"
